...
WFnone <- function(form, train, test, learner, learner.pars){
  ...
  t.trts <- as.numeric(Sys.time() - tm, units = "secs")
  res <- list(trues=responseValues(form, test), preds=preds,
              times = c(traintestT = t.trts, PreTime=PreTime))
  return(res)
}
...
WFRandUnder <- function(form, train, test, learner, rel, thr.rel, C.perc, repl, learner.pars){
  ...
  PreTime <- as.numeric(Sys.time() - pret1, units = "secs")
  tm <- Sys.time()
  preds <- do.call(paste('cv',learner,sep='.'),
...
WFRandOver <- function(form, train, test, learner, rel, thr.rel, C.perc, repl, learner.pars){
  pret1 <- Sys.time()
  newtr <- RandOverRegress(form, train, rel, thr.rel, C.perc, repl)
  PreTime <- as.numeric(Sys.time() - pret1, units = "secs")
  tm <- Sys.time()
  preds <- do.call(paste('cv',learner,sep='.'),
                   list(form,newtr,test, learner.pars))
...
WFGN <- function(form, train, test, learner, rel, thr.rel, C.perc, pert, repl, learner.pars){
  pret1 <- Sys.time()
...
cv.randomForest <- function(form,train,test,learner.pars) {
  ...
  m <- randomForest(form,train, ntree=ntree)
  predict(m,test)
}
...
eval.stats <- function(trues, preds, times, train, metrics,
  ...
  # aucpr <- util(preds,trues,pc,lossF.args,util.control(umetric="AUCPR",event.thr=thr.rel, p=p))
  # aucroc <- util(preds,trues,pc,lossF.args,util.control(umetric="AUCROC",event.thr=thr.rel, p=p)) 
  Ttime <- round(mean(times[["traintestT"]]), 4)
  PreTime <- round(mean(times[["PreTime"]]), 4)
